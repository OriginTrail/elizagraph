...
export const dkgInsert: Action = {
    ...
    handler: async (
        ...
        let recentMessages = message.content.text;
        ...
        if (recentMessages.length >= MIN_CONTENT_LENGTH) {
            ...
            let similarMemoriesQueryResult;
            try {
                const similarMemoriesQuery =
                    getSimilarMemoriesQuery(searchText);
                similarMemoriesQueryResult = await DkgClient.graph.query(
                ...
                elizaLogger.error("Error querying similar memories:", error);
                // Continue execution even if query fails
            }

            // Evaluate if post contains useful knowledge about OriginTrail ecosystem
            const evaluationContext = `Evaluate if the following message contains useful knowledge about OriginTrail, Decentralized Knowledge Graphs (DKG), AI, Knowledge Graphs, paranets, dRAG or blockchain technology.
