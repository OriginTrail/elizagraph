...
import { edgeNodeService } from "../edge-node-helper.ts";
...
export const dkgInsert: Action = {
    ...
    handler: async (
        ...
        elizaLogger.log(`KA: ${JSON.stringify(memoryKnowledgeGraph, null, 2)}`);

        const loginResponse = await edgeNodeService.post(
            `${process.env.EDGE_NODE_AUTH_SERVICE_ROUTE}/login`,
            {
        ...
        // to do: derive edge_node_backend_endpoint drag_endpoint kmining_endpoint

        // Create a Blob from the JSON object and convert it to a File
        const jsonBlob = new Blob(
            [JSON.stringify({ public: memoryKnowledgeGraph })],
            {
                type: "application/json",
            },
        );
        const formData = new FormData();
        formData.append("file", jsonBlob, "memory.json");

        const importDatasetResult = await edgeNodeService.post(
            `${process.env.EDGE_NODE_API_SERVICE_ROUTE}/knowledge-bank/datasets/import`,
            formData,
            {
                headers: {
        ...
        if (importDatasetResult.data.assets) {
            const apiCreateKAResult = await edgeNodeService.post(
                `${process.env.EDGE_NODE_API_SERVICE_ROUTE}/knowledge-bank/assets/create`,
                {
                    knowledgeAssets: importDatasetResult.data.assets.map(
                        (ka) => ({
                            ...ka,
            ...
            createAssetResult = apiCreateKAResult.data.data;
        } else {
            elizaLogger.error("Failed to import dataset");
        }
